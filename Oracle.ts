...
At what scale must a practitioner operate to develop this capacity? → Solid
Which School owns this capacity most naturally? → School
Which slot within that School+Solid? → assign the next available slot for that combination, or slot 1 if unknown
Produce the geocode.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { candidate, description } = req.body as {
    candidate: string;
    description?: string;
  };
